...
function determineWinnerOneFlag(flagPart, resultados) {
	...
	if (stat === "mana") {
		if (resultados[0][1][1] < resultados[1][1][1]) {
			return _nicks[_nicks.indexOf(resultados[1][0])];
		} else if (resultados[0][1][1] > resultados[1][1][1]) {
			return _nicks[_nicks.indexOf(resultados[0][0])];
		}
	} else if (stat === "vida") {
		...
		if (resultados[playerIndex][1][2] < resultados[otherPlayerIndex][1][1]) { // Correct index for vida
			return _nicks[_nicks.indexOf(resultados[otherPlayerIndex][0])];
		} else if (resultados[playerIndex][1][2] > resultados[otherPlayerIndex][1][1]) { // Correct index for vida
			return _nicks[_nicks.indexOf(resultados[playerIndex][0])];
		}
	}
...
async function handleOneFlag(a99, resultados, telefones, juizes_e_os_players_da_luta, quem_enviou_a_mensagem_anterior__juiz, nicks_e_seus_telefones, data_hora_da_ultima_tabela, dia, mes, ano, nicks, data_hora_objeto, dia_semana_numero, aux_dia_semana_numero, nicks_e_suas_quantidade_de_vitorias, datas_nicks_e_suas_quantidade_de_vitorias, meses_e_nicks_com_suas_quantidade_de_vitorias, periodo_mensal, periodo_semanal, _nicks, registrarPontosHelper) {
...
function handleTwoFlags(a99, resultados, telefones, juizes_e_os_players_da_luta, quem_enviou_a_mensagem_anterior__juiz, nicks_e_seus_telefones, data_hora_da_ultima_tabela, dia, mes, ano, nicks, data_hora_objeto, dia_semana_numero, aux_dia_semana_numero, nicks_e_suas_quantidade_de_vitorias, datas_nicks_e_suas_quantidade_de_vitorias, meses_e_nicks_com_suas_quantidade_de_vitorias, periodo_mensal, periodo_semanal, _nicks) {

...
async def find_whatsapp_groups_async(keyword, browser):
	...
	def contar_QuantoS_LinkS_Encontrados():
		"""Conta quantos links foram encontrados."""
		# Lê todos os links já salvos para a palavra-chave na pasta groupsWpp/{palavra}
		existing_links = set()
		term_dir = os.path.join(groups_dir, keyword)
		if os.path.exists(term_dir):
			for fname in os.listdir(term_dir):
				if fname.endswith(".groups.db"):
					with open(os.path.join(term_dir, fname), "r", encoding="utf-8") as f:
						for line in f:
							existing_links.add(line.strip())
		return len(existing_links)
	start = contar_QuantoS_LinkS_Encontrados()
